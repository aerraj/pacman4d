...
export type GameTheme = 'classic' | 'halloween';
...
export class GameEngine {
  ...
  setTheme = (theme:GameTheme) => { this.theme=theme; };
  ...
  private draw(now:number){const rect=this.canvas.getBoundingClientRect(),w=rect.width,h=rect.height,ctx=this.ctx,haunted=this.theme==='halloween';ctx.save();if(now<this.shakeUntil)ctx.translate((this.rng.next()-.5)*8,(this.rng.next()-.5)*8);ctx.clearRect(-20,-20,w+40,h+40);const bg=ctx.createRadialGradient(w*.48,h*.42,0,w*.5,h*.5,Math.max(w,h)*.7);bg.addColorStop(0,haunted?'#171129':'#0c1720');bg.addColorStop(.5,haunted?'#090b18':'#070d13');bg.addColorStop(1,haunted?'#020306':'#030507');ctx.fillStyle=bg;ctx.fillRect(0,0,w,h);const tile=Math.min((w-40)/COLS,(h-28)/ROWS),ox=(w-tile*COLS)/2,oy=(h-tile*ROWS)/2;if(haunted)this.drawHauntedAtmosphere(w,h,now);ctx.translate(w/2,h/2);ctx.transform(1,-.018,.065,.955,0,0);ctx.translate(-w/2,-h/2);this.drawThreatField(tile,ox,oy);this.drawMaze(1-this.player.brane,tile,ox+tile*.32,oy+tile*.38,.12);this.drawMaze(this.player.brane,tile,ox,oy,1);if(haunted)this.drawGhostHouse(tile,ox,oy,now);this.drawReinforcements(tile,ox,oy,now);this.drawSight(tile,ox,oy);
