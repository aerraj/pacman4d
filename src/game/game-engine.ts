...
export class GameEngine {
  ...
  private draw(now:number){const rect=this.canvas.getBoundingClientRect(),w=rect.width,h=rect.height,ctx=this.ctx;ctx.save();if(now<this.shakeUntil)ctx.translate((this.rng.next()-.5)*8,(this.rng.next()-.5)*8);ctx.clearRect(-20,-20,w+40,h+40);const bg=ctx.createRadialGradient(w*.48,h*.42,0,w*.5,h*.5,Math.max(w,h)*.7);bg.addColorStop(0,'#171129');bg.addColorStop(.48,'#090b18');bg.addColorStop(1,'#020306');ctx.fillStyle=bg;ctx.fillRect(0,0,w,h);const tile=Math.min((w-40)/COLS,(h-28)/ROWS),ox=(w-tile*COLS)/2,oy=(h-tile*ROWS)/2;this.drawHauntedAtmosphere(w,h,now);ctx.translate(w/2,h/2);ctx.transform(1,-.018,.065,.955,0,0);ctx.translate(-w/2,-h/2);this.drawThreatField(tile,ox,oy);this.drawMaze(1-this.player.brane,tile,ox+tile*.32,oy+tile*.38,.12);this.drawMaze(this.player.brane,tile,ox,oy,1);this.drawGhostHouse(tile,ox,oy,now);this.drawReinforcements(tile,ox,oy,now);this.drawSight(tile,ox,oy);
  ...
  private drawMaze(brane:number,t:number,ox:number,oy:number,alpha:number){const ctx=this.ctx,depth=t*.2;ctx.save();ctx.globalAlpha=alpha;ctx.lineWidth=Math.max(.7,t*.055);const stroke=brane===0?'rgba(219,146,255,.86)':'rgba(105,226,213,.72)';ctx.strokeStyle=stroke;ctx.shadowBlur=alpha>.5?8:0;ctx.shadowColor=stroke;this.maps[brane].forEach((row,y)=>[...row].forEach((c,x)=>{if(c==='#'){const px=ox+x*t+t*.12,py=oy+y*t+t*.12,w=t*.76,h=t*.76;ctx.fillStyle=brane===0?'rgba(43,24,57,.88)':'rgba(17,52,55,.65)';ctx.beginPath();ctx.moveTo(px+w,py);ctx.lineTo(px+w+depth,py+depth);ctx.lineTo(px+w+depth,py+h+depth);ctx.lineTo(px+w,py+h);ctx.closePath();ctx.fill();ctx.fillStyle=brane===0?'rgba(126,59,148,.26)':'rgba(38,118,116,.18)';ctx.fillRect(px+depth,py+depth,w,h);ctx.strokeRect(px,py,w,h);ctx.beginPath();ctx.moveTo(px,py);ctx.lineTo(px+depth,py+depth);ctx.lineTo(px+w+depth,py+depth);ctx.lineTo(px+w,py);ctx.stroke();}else if(this.dots[brane].has(`${x},${y}`)){ctx.fillStyle=c==='o'?'#ffbf68':'#8fffea';ctx.shadowBlur=c==='o'?13:6;ctx.beginPath();ctx.ellipse(ox+(x+.5)*t,oy+(y+.55)*t,c==='o'?t*.19:Math.max(1,t*.06),c==='o'?t*.13:Math.max(.7,t*.035),0,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;}}));ctx.restore();}
  ...
  private drawGhostHouse(t:number,ox:number,oy:number,now:number){const ctx=this.ctx,left=ox+10.2*t,right=ox+17.8*t,top=oy+11.05*t,bottom=oy+16.05*t,cx=(left+right)/2,pulse=.72+Math.sin(now*.004)*.18;ctx.save();ctx.fillStyle='rgba(8,3,14,.78)';ctx.strokeStyle=`rgba(214,125,255,${pulse})`;ctx.lineWidth=Math.max(1,t*.07);ctx.shadowColor='#b752e5';ctx.shadowBlur=t*.6;ctx.beginPath();ctx.moveTo(left,top+t*1.4);ctx.lineTo(cx,top-t*.75);ctx.lineTo(right,top+t*1.4);ctx.lineTo(right,bottom);ctx.lineTo(left,bottom);ctx.closePath();ctx.fill();ctx.stroke();ctx.shadowBlur=0;ctx.fillStyle='rgba(245,178,78,.2)';for(const x of [left+t*1.2,right-t*1.75]){ctx.fillRect(x,top+t*1.7,t*.55,t*.72);ctx.strokeRect(x,top+t*1.7,t*.55,t*.72);ctx.beginPath();ctx.moveTo(x+t*.275,top+t*1.7);ctx.lineTo(x+t*.275,top+t*2.42);ctx.moveTo(x,top+t*2.06);ctx.lineTo(x+t*.55,top+t*2.06);ctx.stroke();}ctx.fillStyle='rgba(4,1,8,.9)';ctx.beginPath();ctx.moveTo(cx-t*1.05,bottom);ctx.lineTo(cx-t*.92,top+t*2.2);ctx.quadraticCurveTo(cx,top+t*1.28,cx+t*.92,top+t*2.2);ctx.lineTo(cx+t*1.05,bottom);ctx.closePath();ctx.fill();ctx.strokeStyle=`rgba(255,100,204,${.55+pulse*.3})`;ctx.setLineDash([t*.22,t*.16]);ctx.beginPath();ctx.moveTo(ox+12*t,oy+12.48*t);ctx.lineTo(ox+16*t,oy+12.48*t);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle='rgba(235,214,246,.74)';ctx.font=`${Math.max(7,t*.38)}px Georgia`;ctx.textAlign='center';ctx.fillText('SPECTRAL HOUSE',cx,top+t*.62);ctx.strokeStyle='rgba(224,210,235,.22)';for(const side of [-1,1]){const x=side<0?left:right;ctx.beginPath();ctx.moveTo(x,top+t*1.38);ctx.lineTo(x+side*t*.9,top+t*.55);ctx.moveTo(x,top+t*1.38);ctx.lineTo(x+side*t*1.05,top+t*1.32);ctx.stroke();}ctx.restore();}
