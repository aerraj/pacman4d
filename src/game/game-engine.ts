...
export class GameEngine {
  ...
  private draw(now:number){const rect=this.canvas.getBoundingClientRect(),w=rect.width,h=rect.height,ctx=this.ctx;ctx.save();if(now<this.shakeUntil)ctx.translate((this.rng.next()-.5)*8,(this.rng.next()-.5)*8);ctx.clearRect(-20,-20,w+40,h+40);const bg=ctx.createRadialGradient(w*.48,h*.42,0,w*.5,h*.5,Math.max(w,h)*.7);bg.addColorStop(0,'#0c1720');bg.addColorStop(.52,'#070d13');bg.addColorStop(1,'#030507');ctx.fillStyle=bg;ctx.fillRect(0,0,w,h);const tile=Math.min((w-40)/COLS,(h-28)/ROWS),ox=(w-tile*COLS)/2,oy=(h-tile*ROWS)/2;ctx.translate(w/2,h/2);ctx.transform(1,-.018,.065,.955,0,0);ctx.translate(-w/2,-h/2);this.drawThreatField(tile,ox,oy);this.drawMaze(1-this.player.brane,tile,ox+tile*.32,oy+tile*.38,.12);this.drawMaze(this.player.brane,tile,ox,oy,1);this.drawReinforcements(tile,ox,oy,now);this.drawSight(tile,ox,oy);
  ...
  private drawMaze(brane:number,t:number,ox:number,oy:number,alpha:number){const ctx=this.ctx,depth=t*.2;ctx.save();ctx.globalAlpha=alpha;ctx.lineWidth=Math.max(.7,t*.055);const stroke=brane===0?'rgba(221,178,87,.86)':'rgba(105,226,213,.72)';ctx.strokeStyle=stroke;ctx.shadowBlur=alpha>.5?7:0;ctx.shadowColor=stroke;this.maps[brane].forEach((row,y)=>[...row].forEach((c,x)=>{if(c==='#'){const px=ox+x*t+t*.12,py=oy+y*t+t*.12,w=t*.76,h=t*.76;ctx.fillStyle=brane===0?'rgba(58,42,17,.76)':'rgba(17,52,55,.65)';ctx.beginPath();ctx.moveTo(px+w,py);ctx.lineTo(px+w+depth,py+depth);ctx.lineTo(px+w+depth,py+h+depth);ctx.lineTo(px+w,py+h);ctx.closePath();ctx.fill();ctx.fillStyle=brane===0?'rgba(128,92,30,.22)':'rgba(38,118,116,.18)';ctx.fillRect(px+depth,py+depth,w,h);ctx.strokeRect(px,py,w,h);ctx.beginPath();ctx.moveTo(px,py);ctx.lineTo(px+depth,py+depth);ctx.lineTo(px+w+depth,py+depth);ctx.lineTo(px+w,py);ctx.stroke();}else if(this.dots[brane].has(`${x},${y}`)){ctx.fillStyle=c==='o'?'#e9c875':'#72f6e5';ctx.shadowBlur=c==='o'?12:5;ctx.beginPath();ctx.ellipse(ox+(x+.5)*t,oy+(y+.55)*t,c==='o'?t*.19:Math.max(1,t*.06),c==='o'?t*.13:Math.max(.7,t*.035),0,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;}}));ctx.restore();}
